...
KAGGLE_EPISODES=${KAGGLE_EPISODES:-1000}
...
run_train() {
    ...
    local gpu=$2
    local log=$3
    local -a cmd=("$PY" scripts/train.py --exp "$exp" --steps "$STEPS"
...
if [[ -n "$GPU_A" ]]; then
    CUDA_VISIBLE_DEVICES="$GPU_A" "$PY" scripts/compare.py --games "$COMPARE_GAMES" \
        --kaggle --kaggle-episodes "$KAGGLE_EPISODES" 2>&1 | tee results/compare.log
else
    "$PY" scripts/compare.py --games "$COMPARE_GAMES" \
        --kaggle --kaggle-episodes "$KAGGLE_EPISODES" 2>&1 | tee results/compare.log
fi
