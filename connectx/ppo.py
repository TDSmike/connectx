...
from .networks import ActorCritic, NetPolicy, masked_categorical
...
class PPOAgent:
    ...
    def load(self, path: str):
        ...
        a = d["arch"]
        self.net = ActorCritic(a["channels"], a["hidden"], a["res_blocks"]).to(self.device)
        self.net.load_state_dict(d["state_dict"])
        if "config" in d:
            self.cfg = PPOConfig(**d["config"])
        return self
